import { getFaqCategoriesAdmin, getFaqQuestionsAdmin } from "@/services/faq.service";
...
export const useFaqQuestionStore = defineStore("faqQuestion", {
  state: () => ({
    faqQuestion: [],
    faqAnswer: null,
    loading: false, // Loading state
  ...
  actions: {
    async fetchFaqQuestions(forceRefresh = false) {
      // If data is already fetched and no refresh is forced, return early
      if (this.fetched && !forceRefresh) {
        return this.faqCategory;
      }
      ...
      this.loading = true;
      try {
        const { data } = await getFaqQuestionsAdmin({ page: this.currentPage });
        this.faqQuestion = data.resData.data || [];
        this.faqAnswer = data.resData.faq_answers || null;
        this.totalRecords = data.resData.meta.totalCount;
        this.totalPages = data.resData.meta.totalPages;
        this.currentPage = data.resData.meta.page;
        this.error = null;
