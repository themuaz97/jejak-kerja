  ...
  try {
    ...
    const jobApplication = await prisma.job_application.create({
      data: {
        ...
        platform,
        location,
        expected_salary: parseFloat(expectedSalary) || 0,
        offered_salary: parseFloat(offeredSalary) || 0,
        application_status: { connect: { id: parseInt(applyStatusId) } },
        ...(applyOverallId && { application_overall: { connect: { id: parseInt(applyOverallId) } } }),
        remarks: remarks || "",
  ...
  try {
    ...
    const jobApplication = await prisma.job_application.update({
      ...
      data: {
        ...
        ...(applyOverallId && { application_overall: { connect: { id: parseInt(applyOverallId) } } }),
        remarks,
        users_updated_by: { connect: { id: req.user.user_id } },
        updated_at: new Date(),
      },
    });
